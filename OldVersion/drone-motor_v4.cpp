...
//==========================  Main Loop ==============================
	...
	while ( true ) {
		...
		while ( true ) {
			...
			float rpitch    = stickLy * 5.0;
			float rroll     = stickLx * 5.0;
			...
			*/
			...
			float uroll  = 0.005 * rollerr  + 0.1   * ( rollerr  - prollerr[cnt]  ) + 0.00000001 * srollerr;
			float upitch = 0.01  * pitcherr + 0.2   * ( pitcherr - ppitcherr[cnt] ) + 0.00000001 * spitcherr;
			...
			if ( uthrottle < 1.05 ) {
				...
				R = uthrottle - uroll  + uyaw + 0.08;
				L = uthrottle + uroll  + uyaw + 0.05;
				F = uthrottle + upitch - uyaw;
				B = uthrottle - upitch - uyaw + 0.11;
			}
			else {
