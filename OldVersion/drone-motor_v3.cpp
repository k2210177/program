...
//==========================  Main Loop ==============================
	...
	while ( true ) {
		...
		while ( true ) {
			...
			float uroll  = 0.01 * rollerr  + 0.2  * ( rollerr  - prollerr[cnt]  ) + 0.000001 * srollerr;
			float upitch = 0.01 * pitcherr + 0.4  * ( pitcherr - ppitcherr[cnt] ) + 0.000001 * spitcherr;
			float uyaw   = 0.02 * yawerr   + 0.01 * ( yawerr   - pyawerr[cnt]   );

			if ( stickLx <= 0.05 && stickLx >= -0.05 ) uroll  = uroll  - 0.033 * roll;
			if ( stickLy <= 0.05 && stickLy >= -0.05 ) upitch = upitch - 0.033 * pitch;
			...
			else if ( uthrottle >= 1.05 && uthrottle < 1.3 ) {
				R = uthrottle - uroll  + uyaw + 0.08;
				L = uthrottle + uroll  + uyaw + 0.05;
				F = uthrottle + upitch - uyaw + 0.11;
